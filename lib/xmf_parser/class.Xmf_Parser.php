...
class Xmf_Parser {
		...
		if( isset($node->attribute['class']) ) {
			...
			foreach($classes as $class) {
				...
				if( isset( $mf_root[ $class ] ) && ! in_array($node->line ."-" . $node->column . "-".  $class, $this->toskip) ) {
					//This is due to the sudden changes in PHP
					$dummy = ""; $dummy2 = "";
					//Start constructing the result.
					$temp_result = $this->parse( $mf_root[ $class ]['childs'], $node, $dummy, $dummy2  );
	 ...
	 */
	private function parse($mf_struct, $node, &$temp_result, $skip = false) {
			...
			foreach($class_names as $class ) {
				...
				if ( isset( $mf_struct[ $class ] )   && ( (!in_array($node->line ."-" . $node->column . "-".  $class, $this->toskip)) || (isset($mf_struct[$class]['letpass']))) ) {
					...
					if( isset($mf_struct[ $class ]['childs']) ) {
						...
						//Check of Ocurrences
						if( (!$mf_struct == "") && ($mf_struct[ $class ]['ocurrences'] == '1' || $mf_struct[ $class ]['ocurrences'] == '?') ) {			
							//mf property can have only one ocurrence.
							if($temp_result =="" || !isset( $temp_result[ $class ]['node'] ) ) {
